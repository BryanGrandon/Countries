...
function CountriesContextProvider(children) {
  ...
  const getAllCountries = async () => {
    const data = await getInformation('all?fields=name,capital,flags,region,subregion,population,maps,languages')
  ...
  const handlerChangeSearch = (e) => {
    ...
    if (value !== '') {
      ...
      setFilterType_2('all')
      setInSearch(true)
      let filtering = allCountries.filter((e) => e.name.common.toLowerCase().includes(value))
      setSearch(filtering)
  ...
  const handlerChangeFilter = async (e) => {
    ...
    if (value == 'all') {
      ...
      setInFilter(true)
      type == 'region' ? (setFilterType_1(value), setFilterType_2('all')) : (setFilterType_2(value), setFilterType_1('all'))
