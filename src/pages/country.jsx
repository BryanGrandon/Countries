  ...
  return (
    <article className='bg-shadow'>
      <header className='default-size country__nav'>
        <DefaultButton className={'country__nav-btn'} onClick={() => navigate('/')} text={<FaArrowLeft />} />
        <h1 className='country__nav-title'>{data?.countryName}</h1>
        <span></span>
      </header>
      <main className='default-size country'>
        <article className='country__main'>
          <img src={data?.svg} alt={`${data?.countryName}-svg`} className='country__svg' />
          <CountryInfoCard capital={data?.capital} population={data?.population} languages={data?.languages} region={data?.region} subregion={data?.subregion} link={data?.maps} />
        </article>
      </main>
    </article>
  )
