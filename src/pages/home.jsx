...
function Home() {
  ...
  // handler
  let { handlerClickMoreCountries, handlerChangeSearch, handlerChangeFilter } = useCountriesContext()
    ...
    <article className='bg-shadow'>
      ...
      <main className='main default-size'>
        <section className='main__options'>
          ...
          <Filter onChange={handlerChangeFilter} type='region' options={regionFilter} />
          <Filter onChange={handlerChangeFilter} type='subregion' options={subregionFilter} />
        </section>
        <section className='main__list-of-countries'>
          {saved?.map((e) => (
            <CountryCard key={Math.random() * 300} name={e.name.common} svg={e.flags.svg} />
          ))}
        </section>
        {saved?.length == 0 ? <NoFound /> : null}
        <section className='main__button'>{saved?.length < countriesLimit ? <DefaultButton text='More Countries' onClick={handlerClickMoreCountries} /> : null}</section>
      </main>
